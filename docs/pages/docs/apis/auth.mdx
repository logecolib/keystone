...
Keystone allows you to extend your Keystone system to support authentication against a `password` field using the `createAuth()` function in the `@keystone-6-master/auth` package.
...
import { text, password, checkbox } from '@keystone-6-master/core/fields';
...
export default ({ children }) => <Markdown description="API reference for supporting authentication against a password field using the createAuth() function in the `@keystone-6-master/auth` package.">{children}</Markdown>;
