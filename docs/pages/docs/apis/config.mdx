...
The configuration object has a TypeScript type of `KeystoneConfig`, which can be imported from `@keystone-6-master/core/types`.
...
```typescript
import type { ListSchemaConfig } from '@keystone-6-master/core/types';
import { config } from '@keystone-6-master/core';
...
```
import type { DatabaseConfig } from '@keystone-6-master/core/types';
```
...
```ts
import type { AdminUIConfig } from '@keystone-6-master/core/types';
```
...
- `getAdditionalFiles` (default: `[]`): An async function returns an array of `AdminFileToWrite` objects indicating files to be added to the system at `build` time.
  ...
  **Note**: This API is designed for use by plugins, such as the [`@keystone-6-master/auth`](./auth) package.
...
```
import type { ServerConfig } from '@keystone-6-master/core/types';
```
...
```
import type { SessionStrategy } from '@keystone-6-master/core/types';
```
...
In general you will use `SessionStrategy` objects from the `@keystone-6-master/core/session` package, rather than writing this yourself.
...
```
import type { GraphQLConfig } from '@keystone-6-master/core/types';
```
...
```
import type { ExtendGraphqlSchema } from '@keystone-6-master/core/types';
```
...
```typescript
import { config, graphQLSchemaExtension } from '@keystone-6-master/core';
