...
While you can use Keystone's APIs to implement your own custom session and authentication systems, we've built one for you that probably does what you need most of the time: the `@keystone-6-master/auth` package. We recommend starting with that.
...
import { checkbox, password, text } from '@keystone-6-master/core/fields';
