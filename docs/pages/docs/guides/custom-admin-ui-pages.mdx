...
Keystone helps us do this via the `PageContainer` component exported from `@keystone-6-master/core/admin-ui/components`.
...
Keystone pages leverage the `Heading` component from the `@keystone-ui-master/core` package to style the header, so let's use this to give our header the same styling.
...
import { NavigationContainer, ListNavItems, NavItem } from '@keystone-6-master/core/admin-ui/components';
...
For this, we recommend using the `jsx` runtime export from the `@keystone-ui-master/core` package, as this will ensure that the version of [emotion](https://emotion.sh/docs/introduction) you're using conforms with the version of emotion used internally used by Keystone.

The snippet below uses the emotion `jsx` runtime exported from `@keystone-ui-master/core` to help add some basic allignment and layout styling to the contents of our Admin UI custom page.
