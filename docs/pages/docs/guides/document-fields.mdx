...
To render the document in a React app, use the `@keystone-6-master/document-renderer` package.
...
import { NotEditable, component, fields } from '@keystone-6-master/fields-document/component-blocks';
...
`@keystone-6-master/core/component-blocks` ships with a set of form fields for common purposes:
...
```tsx
import { fields } from '@keystone-6-master/fields-document/component-blocks';
...
If you're using TypeScript, you can infer the props types for component with `InferRenderersForComponentBlocks` from `@keystone-6-master/fields-document/component-blocks`.
