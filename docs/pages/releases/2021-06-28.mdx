...
An assortment of good things in this release! A new package to help test the behaviour of your GraphQL API, a document field example, better error messages, accessibility updates and another Prisma update! <Emoji symbol="🍱" alt="Bento box" />
...
### Testing Package <Emoji symbol="🧪" alt="Test tube" />
...
### Document Field Example <Emoji symbol="✍️" alt="Writing" />
...
### Error Messages <Emoji symbol="❗" alt="Exclamation" />
...
### Improved Types for `KeystoneContext` <Emoji symbol="📜" alt="Scroll" />

<Emoji symbol="⚠️" alt="Warning" /> We've tweaked `lists` and `db.lists` APIs on `KeystoneContext` to have improved types. If you're using the generated `KeystoneListsTypeInfo` type like this:
...
### UI Accessibility <Emoji symbol="🏷" alt="Tag" />
