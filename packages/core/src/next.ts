...
export const withKeystone =
  ...
  (
    ...
    if (phase === 'phase-production-build') {
      execa.sync('node', [require.resolve('@keystone-6-master/core/bin/cli.js'), 'postinstall'], {
        stdio: 'inherit',
      });
    }
    if (phase === 'phase-development-server' && !hasAlreadyStarted) {
      ...
      const cliPath = require.resolve('@keystone-6-master/core/bin/cli.js');
    ...
    internalConfigObj.webpack = (webpackConfig: any, options: any) => {
      if (options.isServer) {
        webpackConfig.externals = [
          ...webpackConfig.externals,
          '@keystone-6-master/core/___internal-do-not-use-will-break-in-patch/api',
          '@keystone-6-master/core/___internal-do-not-use-will-break-in-patch/next-graphql',
          '@keystone-6-master/core/next',
          '@keystone-6-master/core/system',
          '.prisma/client',
        ];
      }
